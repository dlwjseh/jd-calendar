...
- **Componentize aggressively.** Each visible UI piece should be its own
  `struct ...: View` in its own file (`JDCalendar/<Name>.swift`), so a future
  edit touches one file. Rules of thumb:
...
JDCalendar/                   # synced folder — adding .swift files needs no pbxproj edit
  JDCalendarApp.swift         # @main, WindowGroup (1100×720, min 720×540)
  ContentView.swift           # page background + card wrapper + cursor (year/month) state
  ...
  CalendarTheme.swift         # design tokens (light only)
...
- Quick syntax check from CLI:
  `swiftc -typecheck -sdk "$(xcrun --show-sdk-path --sdk macosx)" JDCalendar/*.swift`
